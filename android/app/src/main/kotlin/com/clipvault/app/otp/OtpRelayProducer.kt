...
private const val OTP_NONCE_BYTES = 12
private const val OTP_TAG_BYTES = 16
private const val OTP_PAIR_VERIFIER_BYTES = 32
...
private fun canonicalUuid4(value: String, name: String): UUID {
    val parsed = try {
...
private fun canonicalDevice(value: String, name: String): UUID {
...
/** Metadata-only proof presented to a reviewed platform capture adapter. */
...
/** Single-owner OTP characters emitted without the surrounding message body. */
class IsolatedOtpCandidate(
    val source: OtpCaptureSource,
    val grantId: String,
    val targetDeviceId: String,
    ownedDigits: CharArray,
) : AutoCloseable {
    private var digits = ownedDigits
...
/** Disabled by default; no SMS or Autofill capability is fabricated here. */
...
class DisabledOtpCapturePort : OtpCapturePort {
    override val source = OtpCaptureSource.SMS_CODE_AUTOFILL
    override fun capture(authorization: OtpCaptureAuthorization): IsolatedOtpCandidate? = null
}

/**
 * One ownership-transfer view of OTP-only pair state.
 *
 * The 32-byte verifier is supplied by the reviewed pair authority. It is not
 * the ordinary sync bearer and is wiped when this lease closes.
 */
class OtpPairMaterialLease(
    ...
    init {
        try {
            ...
            require(ownedPairVerifier.size == OTP_PAIR_VERIFIER_BYTES) {
...
interface OtpPairMaterialPort : AutoCloseable {
    fun acquire(authorization: OtpCaptureAuthorization): OtpPairMaterialLease?
    override fun close() = Unit
}

class DisabledOtpPairMaterialPort : OtpPairMaterialPort {
    override fun acquire(authorization: OtpCaptureAuthorization): OtpPairMaterialLease? = null
}
...
internal fun interface OtpNonceSource {
    fun nextNonce(): ByteArray
}

internal fun interface OtpEventIdSource {
...
private class OtpWireEnvelope(
    ...
    private val authenticationTag: ByteArray,
) : AutoCloseable {
    fun toJsonBytes(): ByteArray {
        ...
        val body = JSONObject()
            ...
            .put("authentication_tag", encoder.encodeToString(authenticationTag))
            .toString()
...
private fun sealOtp(
    ...
    require(nonce.size == OTP_NONCE_BYTES) { "OTP nonce must be 12 bytes" }
    val key = deriveOtpAesKey(
        material.verifier(),
        material.sessionEpoch,
        material.senderDeviceId,
        material.targetDeviceId,
    )
    ...
    try {
        ...
        cipher.init(
            ...
            GCMParameterSpec(OTP_TAG_BYTES * 8, nonce),
        )
        ...
        return OtpWireEnvelope(
            sessionEpoch = material.sessionEpoch,
            eventId = eventId,
            senderDeviceId = material.senderDeviceId,
            targetDeviceId = material.targetDeviceId,
...
/**
 * Runtime-only OTP JCA producer. All platform authorities are injected and
 * default to disabled; construction does not grant SMS, Autofill, or pairing.
 */
class OtpJcaRelayProducer internal constructor(
    ...
    constructor(
        ...
        pairMaterialPort,
        transport,
        SecureRandomOtpNonceSource(),
        OtpEventIdSource { UUID.randomUUID().toString() },
    ...
    private fun reserve(material: OtpPairMaterialLease, nonce: ByteArray): Boolean =
        synchronized(stateLock) {
            ...
            if (scope != nonceScope) {
                ...
                nonceScope?.let { previous ->
                    if (retiredScopes.size >= OTP_RETIRED_SCOPE_CAPACITY) {
                        return@synchronized false
                    }
                    retiredScopes.add(previous)
                }
                nonceScope = scope
                lastSequence = 0L
                nonceDigests.clear()
            }
            if (material.sequence <= lastSequence) return@synchronized false
            if (nonceDigests.size >= OTP_NONCE_HISTORY_CAPACITY) return@synchronized false
    ...
    fun captureAndRelay(
        ...
        synchronized(stateLock) {
            if (closed) return OtpRelaySendStatus.DISABLED
        }
        ...
        return try {
            ...
            isolated = capturePort.capture(authorization)
                ?: return OtpRelaySendStatus.DISABLED
            characters = isolated.take(
                authorization,
                monotonicClockMs(),
                explicitUserAction,
            )
            ...
            for (index in characters.indices) {
                val digit = characters[index]
                if (digit !in '0'..'9') throw OtpRelayRejected("invalid OTP character")
                plaintext[index] = digit.code.toByte()
            }
            material = pairMaterialPort.acquire(authorization)
                ?: return OtpRelaySendStatus.DISABLED
            if (
                ...
                throw OtpRelayRejected("pair material mismatch")
            }
            if (ttlMs !in 1..OTP_RELAY_MAX_TTL_MS) {
                throw OtpRelayRejected("invalid OTP TTL")
            }
            ...
            if (nonce.size != OTP_NONCE_BYTES || !reserve(material, nonce)) {
                throw OtpRelayRejected("nonce or sequence reuse rejected")
            }
            envelope = sealOtp(
                plaintext,
                material,
