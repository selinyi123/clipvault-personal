...
# Read the public web-signing constant from RSSHub's maintained Sohu implementation.
...
print('BOOTSTRAP',json.dumps({'profile_status':pr.status_code,'profile_bytes':len(profile_html),'pv_id':PV_ID,'has_suv':bool(suv_value)},ensure_ascii=False),flush=True)
...
def request_page(page,cookie_mode='standard',diagnostic=False):
    ...
    for attempt in range(4):
        try:
            ...
            r=client().post(ODIN,json=body,headers={'Cookie':cookie_header(cookie_mode,now)},timeout=30)
            r.raise_for_status()
            payload=r.json()
            ...
            if diagnostic:
                diag={
                    ...
                    'node_list_count':len(node.get('list') or []),
                }
...
# Try the request-context variants explicitly and select the first one that returns real feed items.
variant_results=[]
selected_mode=None
...
if selected_mode:
    ...
    pages[1]=p1[1]; pages[2]=p2[1]
    if p1[2]: errors[1]=p1[2]
    if p2[2]: errors[2]=p2[2]
else:
    p1=(1,[],None,None); p2=(2,[],None,None)
...
probe={
    ...
    'overlap':len({x['id'] for x in p1[1]} & {x['id'] for x in p2[1]}),
}
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if done%25==0 or done==MAX_PAGE:
                print('PROGRESS',done,MAX_PAGE,'errors',len(errors),flush=True)
...
nonempty=sorted(p for p,v in pages.items() if v)

summary={
    'status':'complete' if selected_mode and not errors else ('complete_with_errors' if selected_mode else 'probe_failed'),
    'profile_status':pr.status_code,
    'pv_id':PV_ID,
    'has_suv':bool(suv_value),
    'probe':probe,
    'pages_requested':MAX_PAGE,
    'pages_with_items':len(nonempty),
    'max_nonempty_page':max(nonempty) if nonempty else None,
    'failed_pages':errors,
    'unique_items':len(all_items),
    'expected_profile_content_count':8448,
    'difference_from_profile_count':len(all_items)-8448,
    'newest_datetime':all_items[0]['datetime'] if all_items else None,
    'oldest_datetime':all_items[-1]['datetime'] if all_items else None,
...
with open('sohu_selected.csv','w',encoding='utf-8-sig',newline='') as f:
    w=csv.DictWriter(f,fieldnames=['date','datetime','title','id','url','page'])
    w.writeheader()
    for x in selected:
