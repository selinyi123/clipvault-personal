...
The frozen v1 canonical version lives in ``desktop/clipvault/__init__.py``
(``__version__``); that is what the running desktop reports over the API.
PyPI metadata and the v1 Inno installer retain matching copies because their
toolchains cannot share the Python constant. This tool aligns those three, and
``desktop/tests/test_release_alignment.py`` fails CI if the frozen v1 runtime,
Python package, and v1 installer drift. Android v2 now reads its independent
candidate version from ``contracts/v2_candidate_version.json``.

Usage:
    python scripts/bump_version.py 1.6.0            # align frozen v1 metadata
...
def bump(version: str, code: int | None) -> None:
    ...
    if code is not None:
        raise SystemExit(
            "--code is retired: Android v2 is governed by "
    ...
    print(f"set frozen v1 version {version} across init/pyproject/installer")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--code", type=int, default=None, help=argparse.SUPPRESS)
