...
int main() {
  ...
  SetOptionRequest option{host_id, "session-1", 6, 4, "ascii_mode", true};
  SetOptionRequest decoded_option;
  ok &= Expect(DecodeSetOption(EncodeSetOption(option), &decoded_option) &&
                   decoded_option.request_seq == 6 &&
                   decoded_option.expected_revision == 4 &&
                   decoded_option.option == "ascii_mode" &&
                   decoded_option.enabled,
               "SetOption round trip");
  option.option = "invalid option";
  ok &= Expect(EncodeSetOption(option).empty(),
               "SetOption rejects non-canonical option name");

  EndSessionRequest end{host_id, "session-1", 7};
  EndSessionRequest decoded_end;
  ok &= Expect(DecodeEndSession(EncodeEndSession(end), &decoded_end) &&
                   decoded_end.request_seq == 7,
               "EndSession round trip");
  SessionEnded ended{host_id, "session-1", 7};
  SessionEnded decoded_ended;
  ok &= Expect(DecodeSessionEnded(EncodeSessionEnded(ended), &decoded_ended) &&
                   decoded_ended.ack_request_seq == 7,
               "SessionEnded round trip");
  ResponseAck acknowledgement{host_id, "session-1", 7};
  ResponseAck decoded_acknowledgement;
  ok &= Expect(DecodeResponseAck(EncodeResponseAck(acknowledgement),
                                 &decoded_acknowledgement) &&
                   decoded_acknowledgement.ack_request_seq == 7,
               "authenticated response acknowledgement round trip");
  ...
  ok &= Expect(projection.Begin(host_id, "session-1") &&
                   projection.live_session_count() == 1 &&
                   projection.Reserve(host_id, "session-1", 1) ==
                       ResponseReservation::kReserved &&
                   projection.Reserve(host_id, "session-1", 1) ==
                       ResponseReservation::kDuplicate &&
                   projection.Reserve(host_id, "session-1", 3) ==
                       ResponseReservation::kInvalid &&
                   projection.Reserve(host_id, "session-1", 2) ==
                       ResponseReservation::kReserved,
               "ENG2-V003 bounded reserve-before-editor response ledger");
  projection.End();
  ok &= Expect(projection.live_session_count() == 0,
               "ENG2-V008 projection ledger cleanup on session end");
