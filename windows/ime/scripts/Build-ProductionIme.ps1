...
$x64DebugBuild = Join-Path $buildRoot 'x64-debug'
$x86DebugBuild = Join-Path $buildRoot 'x86-debug'
...
& (Join-Path $PSScriptRoot 'Build-NativeSlice.ps1') `
    -Configuration Debug -Architecture x64 -BuildDirectory $x64DebugBuild `
    -RimeSdkDirectory $RimeSdkDirectory -RimeDataDirectory $rimeData `
    -RequireRime
& (Join-Path $PSScriptRoot 'Build-NativeSlice.ps1') `
    -Configuration Debug -Architecture x86 -BuildDirectory $x86DebugBuild
...
if ($IsccPath) { $installerGate.IsccPath = $IsccPath }
& (Join-Path $PSScriptRoot 'Test-InstallerInclude.ps1') @installerGate
...
$required = @(
    ...
    'scripts\Configure-ClipVaultImeUser.ps1',
    'scripts\Test-ClipVaultInstallerOwner.ps1',
    'scripts\Stop-ClipVaultImeOwnerProcesses.ps1',
