...
enum class FrameKind : std::uint32_t {
  ...
  kSetOptionRequest = 26,
  kEndSessionRequest = 27,
...
struct EndSessionRequest final {
  std::string host_instance_id;
  std::string session_id;
  std::uint64_t request_seq = 0;
};

struct SessionEnded final {
  std::string host_instance_id;
  std::string session_id;
  std::uint64_t ack_request_seq = 0;
};

struct ResponseAck final {
  std::string host_instance_id;
  std::string session_id;
  std::uint64_t ack_request_seq = 0;
};
...
std::vector<std::uint8_t> EncodeResponseAck(const ResponseAck& acknowledgement);
...
enum class ResponseReservation : std::uint8_t {
  kReserved,
  kDuplicate,
  kInvalid,
};

// A TSF PipeEngineClient owns one typed session at a time. This constant-size
// high-water ledger is advanced before any EngineState can reach an editor edit
// session, so a repeated response can never project commit_text twice.
class ResponseProjectionLedger final {
 public:
  bool Begin(const std::string& host_instance_id,
             const std::string& session_id);
  ResponseReservation Reserve(const std::string& host_instance_id,
                              const std::string& session_id,
                              std::uint64_t ack_request_seq) noexcept;
  void End() noexcept;
  [[nodiscard]] std::size_t live_session_count() const noexcept {
    return session_id_.empty() ? 0u : 1u;
  }

 private:
  std::string host_instance_id_;
  std::string session_id_;
  std::uint64_t high_water_mark_ = 0;
};
...
class PipeEngineClient final {
 public:
  ...
  bool CancelComposition(
      ...
      DWORD budget_milliseconds = kDefaultRpcBudgetMilliseconds);
  bool SetOption(
      const std::string& option, bool enabled, EngineState* state,
      DWORD budget_milliseconds = kDefaultRpcBudgetMilliseconds);
  bool EndSession(
      DWORD budget_milliseconds = kDefaultRpcBudgetMilliseconds);
 ...
 private:
  ...
  bool AcceptState(const std::vector<std::uint8_t>& response,
                   EngineState* state, ULONGLONG deadline_tick);
  bool SendResponseAck(std::uint64_t ack_request_seq,
  ...
  ResponseProjectionLedger projection_ledger_;
};
