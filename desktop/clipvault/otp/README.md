...
- Android Runtime now has a vector-matched JCA AES-256-GCM/HKDF producer and
  one-shot HTTP adapter, but still needs a reviewed SMS Code Autofill or
  approved SMS capture adapter, an OTP-only pair-material provider, platform
  key protection and device tests;
- Windows still needs the matching reviewed CNG decrypt/key adapter;
