...
## Production architecture supersession

The same-APK dual-IME rows below are retained as the historical v2.0 milestone
and must not be rewritten as device evidence. They are no longer the production
Android architecture for the v2 daily-use candidate. ADR-0010 and ADR-0013 now
select a standalone, no-network `com.clipvault.ime` package with one system IME
entrypoint plus a separate networked Runtime whose legacy IME services are
removed from the production manifest rather than merely disabled; ADR-0014 and ADR-0016 govern separated candidate surfaces and
Runtime-owned ephemeral OTP capture.

Accordingly, `tools/v2_keyboard_readiness.py` now checks that Android-local
production boundary. The canonical cross-platform automated and Owner evidence
gate is `tools/v2_daily_readiness.py` together with
`docs/V2_DAILY_USE_ACCEPTANCE.md` and `docs/V2_DAILY_USE_MANUAL_QA.md`. Neither
helper may convert unsigned builds, synthetic tests or missing manual evidence
