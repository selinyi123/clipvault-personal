...
| # | Direction | Sources | Key finding | Decision |
|---|---|---|---|---|
| R29 | Pull cursor checkpointing and outbox pruning | CouchDB `_changes` docs (`https://docs.couchdb.org/en/stable/api/database/changes.html`), CouchDB replication protocol docs (`https://docs.couchdb.org/en/stable/replication/protocol.html`), PouchDB API replication/checkpoint docs (`https://pouchdb.com/api.html`) | Mature local-first replication systems expose a response cursor/checkpoint that represents the sequence reached by the returned batch, not the request's previous `since` value. ClipVault already returns `next_seq`, and Android stores it as `sinceSeq`; desktop peer ack tracking should mirror that delivered cursor so pruning and status do not lag behind successful pulls. | **Adopt now:** after `/api/sync/pull` builds a page, record `my_acked_seq` as the returned `next_seq`. This keeps the existing protocol and Android client unchanged, but lets desktop outbox pruning use the same cursor that the peer will request next. |
