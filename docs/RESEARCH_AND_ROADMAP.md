...
| # | Direction | Sources | Key finding | Decision |
|---|---|---|---|---|
| R85 | Downloaded release artifact evidence and provenance | GitHub Artifact Attestations overview (`https://docs.github.com/en/actions/concepts/security/artifact-attestations`), GitHub artifact-attestation generation docs (`https://docs.github.com/actions/security-for-github-actions/using-artifact-attestations/using-artifact-attestations-to-establish-provenance-for-builds`), GitHub CLI `gh attestation verify` manual (`https://cli.github.com/manual/gh_attestation_verify`), GitHub Actions artifacts REST API (`https://docs.github.com/en/rest/actions/artifacts`), GitHub Actions variables REST API (`https://docs.github.com/en/rest/actions/variables`), GitHub Releases REST API (`https://docs.github.com/en/rest/releases/releases`), GitHub CLI `gh run download` manual (`https://cli.github.com/manual/gh_run_download`) | GitHub documents that attestation certificate fields are not workflow-controlled, while predicate metadata can be. Actions and Release assets expose canonical SHA-256 digests, the release-by-tag endpoint is published-only, authenticated release listings expose drafts to users with push access, and environment variables are readable to collaborators with the required permission. | **Adopted:** `tools/release_artifact_evidence.py --require-live-final-draft` verifies the exact successful run attempt, strong per-file attestation identity including certificate `runInvocationURI`, two Actions bundles, exact nine-file Actions/draft parity and API digests (including the LGPL relink kit), the live release-environment certificate variable, and an independent Owner-bound APK signer result. Its read-only `--require-live-published-release` mode repeats those checks after publication, reconstructs the approved pre-publication binding from live state, and binds the exact published Release, asset IDs, and tag object into a separate closure digest. It emits canonical path-free evidence but still does not download artifacts, trigger workflows, publish `v1.6.0`, run manual QA, or authorize Issue #36 completion. |
