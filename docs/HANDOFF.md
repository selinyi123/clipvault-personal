...
- `tools/download_field_test_artifacts.py` is a local Issue #82 candidate
  artifact downloader for the field-test lane. It reads a workflow run's
  artifact inventory with `gh api`, downloads only the expected Windows and
  Android release-candidate artifacts by exact artifact name/id, checks
...
- `tools/v2_keyboard_readiness.py` keeps its historical filename but now reports
  the accepted v2 daily-use Android boundary: the networked Runtime enables no
  IME, the standalone `com.clipvault.ime` APK exposes one system input method,
  and that IME has no network/SMS/notification-listener authority. It also checks
  the single input-method XML, production shell controls, standalone host tests,
  and ADR-0010/0013/0014/0016 plus daily-use acceptance markers. Its only external
  blocker is the Owner signed/manual gate, which is delegated to
  `tools/v2_daily_readiness.py`; it does not synthesize duplicate approval
  evidence, call GitHub, run device QA, sign or publish releases, or claim stable
  daily-use readiness.
