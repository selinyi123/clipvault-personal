...
main HEAD desktop suite: **179 passed** on Linux/CI-portable runners
(`python -m pytest -q --ignore=tests/test_watcher.py --ignore=tests/test_instance_lock.py`,
verified 2026-06-28；含 #19 SG-1.2、#21 SUG-1.3、及本轮 CJK-FTS 7 例）。另有 **4** 项 Windows-only
用例（`test_watcher.py` 3 + `test_instance_lock.py` 1，依赖 `ctypes.WinDLL`，仅 windows-latest CI 可跑）
→ 桌面总计 **183** 项。schema 版本 = **5**（migration 0005：clips_fts 改 trigram 分词器，修中文全文搜索）。
