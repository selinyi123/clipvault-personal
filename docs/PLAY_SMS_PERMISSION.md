...
- The default Runtime implements SMS User Consent through the exact locked official
  `play-services-auth-api-phone:18.2.0` client. It starts only after an explicit tap, is bound to one target
  and a 120-second in-memory session, and requires the system dialog for each message. This fallback does
  not request `READ_SMS` or `RECEIVE_SMS`. SMS Retriever is useful only when the sender format/hash is
  controlled by the participating application; it is not a generic bank/social OTP reader.
...
runtime default
  no restricted SMS permission; Inline Autofill and explicit one-message User Consent only
