...
For the production self-hosted pair lane, the Desktop pair authority generates `pair_verifier` as 32
cryptographically random bytes. The existing sync bearer authenticates the one-time
`POST /api/otp/pair` request but is never reused as an OTP identity, verifier, encryption key, salt or
nonce. The authority returns the verifier exactly once as canonical unpadded base64url over a
user-confirmed loopback or Tailscale path. Windows stores the same raw bytes only in the per-user
Credential Manager `CVPK` record; Android must immediately import them into platform-protected secret
storage and discard the response buffer. SQLite stores routing UUIDs and revocation state only. The
verifier is never sent to a relay or ordinary sync/outbox path. An unauthenticated or ordinary-LAN
cleartext pairing exchange cannot be cited as v2.5 E2EE evidence.

The v2 HTTP relay still uses the existing sync bearer for request admission, so both endpoints enforce
the same transport boundary for `POST /api/otp/pair` and `POST /api/otp/relay`: cleartext HTTP is allowed
only to literal loopback or Tailscale (`100.64.0.0/10`, `fd7a:115c:a1e0::/48`) addresses; a normal LAN
address or cleartext hostname is rejected before the bearer or body is sent/read. HTTPS may use a DNS
name with normal platform certificate validation. Redirects remain disabled. An OTP-only transport token
is a future compartmentalization improvement, not permission to weaken this transport gate.
...
The online HTTP envelope shape is independently frozen by
`contracts/otp_relay_wire_v1.schema.json`. It uses canonical unpadded base64url and keeps the 16-byte
GCM authentication tag separate from the 4-8 byte ciphertext. The JSON representation is transport
framing only: implementations must build the binary AAD above directly and must never authenticate a
locale-dependent JSON serialization.
