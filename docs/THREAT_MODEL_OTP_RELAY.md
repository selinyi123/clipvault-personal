...
| Locked/shared/remote PC leaks code | Suppress presentation/insertion while locked; mask or disable in remote capture contexts |
...
- The v2.5 self-hosted key profile derives from the existing pair-secret verifier. Compromise of either
  paired endpoint or its protected pairing state therefore compromises that pair's OTP channel; it does
  not expose another pair. An unauthenticated cleartext pairing ceremony is outside the E2EE claim.
